...
实验结果表明，调参后的 LightGBM 模型在公开健康检查数据集上具有较好的分类效果，血压相关特征是影响模型判断的主要因素。预测流程验证表明，本文方法能够完成从血压趋势预测、风险分类到结果展示的完整过程，可为用户提供短期健康管理参考，但不能替代临床诊断和治疗决策。
...
本文围绕“Prophet 血压趋势预测 + LightGBM 风险分类 + 预测结果展示”开展研究与实现。研究方法包括文献分析、模型设计、特征构建、实验评估和流程验证。文献分析用于确定高血压风险预测、时间序列预测和机器学习分类的相关基础；模型设计用于明确 Prophet 与 LightGBM 的输入输出关系；特征构建用于说明预测期血压特征如何进入风险分类模型；实验评估用于检验 LightGBM 分类效果、Prophet 预测误差和趋势融合作用；流程验证用于确认两个模型能够在预测页面中完成完整调用。
...
第二，实现预测结果展示。系统以风险因素和历史血压记录作为输入，先由 Prophet 生成未来 7 天收缩压和舒张压趋势，再由 LightGBM 输出高血压风险概率，最后结合趋势信号生成用户可理解的风险结果。
...
第四，完成预测流程验证。系统在预测页面中展示风险概率、风险等级、未来 7 天血压趋势和健康建议，用于说明两个模型的输入输出关系能够在实际系统中完整运行。
...
本章围绕 Prophet 与 LightGBM 两个模型展开算法设计。Prophet 负责从历史血压记录中生成未来 7 天血压趋势，LightGBM 负责结合预测期血压特征和用户风险因素输出风险概率，趋势融合模块在原始概率基础上进行小幅可解释修正。系统页面只用于展示预测入口和预测结果，服务于预测流程验证而不是作为正文主要研究对象。
...
本节对 LightGBM 风险分类模型的训练结果进行分析。为避免概念混淆，本文将两组实验固定命名为对照组和实验组：对照组采用原训练配置，`threshold_search_mode` 为 `recall_priority`，并设置 `threshold_min_recall=0.75`；实验组只将 `threshold_search_mode` 改为 `f1`，不设置固定 Recall 下限。两组实验的数据集、随机种子、训练测试划分、缺失值策略、标签规则、学习率、交叉验证折数和最大训练轮数均保持一致，因此本节比较的是阈值搜索策略对分类结果的影响。

#### 5.2.1 实验分组与模型训练结果

本次实验重新运行训练脚本，分别生成 `baseline-recall-rerun` 和 `experiment-f1-rerun` 两组结果。两组都使用 `Hypertension-risk-model-main.csv` 数据集，共 4240 条样本，正样本比例为 31.1%。模型训练阶段均使用 LightGBMTunerCV 进行参数搜索，调参得到的 CV AUC 均为 0.9527，最佳迭代轮数均为 135。调参后的主要参数也一致，包括 `num_leaves=11`、`feature_fraction=0.9000`、`bagging_fraction=0.9797`、`bagging_freq=7`、`min_child_samples=20`。这说明两组实验的模型主体基本一致，差异集中在最终阈值选择上。

**表5-2 LightGBM 对照实验配置**

| 项目 | 对照组 | 实验组 |
| --- | --- | --- |
| threshold_search_mode | recall_priority | f1 |
| threshold_min_recall | 0.75 | 不设置 |
| 数据集 | Hypertension-risk-model-main.csv | Hypertension-risk-model-main.csv |
| 样本数 | 4240 | 4240 |
| 正样本比例 | 31.1% | 31.1% |
| CV AUC | 0.9527 | 0.9527 |
| TunerCV Best Iteration | 135 | 135 |
| Tuned 模型 Best Iteration | 65 | 65 |

从表 5-2 可以看出，对照组和实验组在数据来源、样本规模和调参结果上保持一致。这样设计实验的好处是比较边界清楚：后续指标差异不是由数据划分、模型参数或特征输入造成的，而是由 `recall_priority` 和 `f1` 两种阈值搜索策略造成的。

#### 5.2.2 核心性能指标对比

两组调参后模型的核心性能指标如表 5-3 所示。AUC、PR-AUC 和 Brier Score 在两组中完全一致，说明模型的概率排序能力和概率误差没有变化；真正变化的是分类阈值，以及阈值变化后带来的 Precision、Recall、F1 和混淆矩阵差异。

**表5-3 LightGBM 阈值策略核心指标对比**

| 指标 | 对照组（recall_priority） | 实验组（f1） | 变化情况 |
| --- | ---: | ---: | --- |
| AUC | 0.9480 | 0.9480 | 不变 |
| PR-AUC | 0.8678 | 0.8678 | 不变 |
| Brier Score | 0.0829 | 0.0829 | 不变 |
| Accuracy | 0.8797 | 0.9021 | 提升 0.0224 |
| Precision | 0.8286 | 0.8125 | 下降 0.0161 |
| Recall | 0.7719 | 0.8897 | 提升 0.1178 |
| F1 | 0.7992 | 0.8494 | 提升 0.0502 |
| Threshold | 0.7732 | 0.6676 | 实验组阈值更低 |
| 最终采用 | 否 | 是 | 采用实验组 |

从表 5-3 可以看出，对照组的阈值为 0.7732，实验组的阈值为 0.6676。对照组阈值更高，模型更谨慎，只有概率较高的样本才会被判定为高风险，因此 Precision 较高。实验组阈值降低后，模型会识别出更多潜在高风险样本，Precision 略有下降，但 Recall 和 F1 提升明显。对于本文系统来说，预测结果用于健康风险提醒，而不是临床确诊。只要 Precision 仍处在可接受范围内，减少高风险样本漏判更符合系统设计目标。

#### 5.2.3 混淆矩阵分析与方案选择

两组实验在测试集上的混淆矩阵如表 5-4 所示。测试集共有 848 个样本，其中正类样本 263 个、负类样本 585 个。

**表5-4 LightGBM 混淆矩阵对比**

| 指标 | 对照组（recall_priority） | 实验组（f1） | 说明 |
| --- | ---: | ---: | --- |
| TN | 543 | 531 | 实验组少正确识别 12 个负类 |
| FP | 42 | 54 | 实验组多误报 12 个 |
| FN | 60 | 29 | 实验组少漏判 31 个 |
| TP | 203 | 234 | 实验组多识别 31 个正类 |
| 漏判率 | 22.81% | 11.03% | 按 263 个正类样本计算 |
| 误报率 | 7.18% | 9.23% | 按 585 个负类样本计算 |

对照组能够正确识别 203 个正类样本，但仍漏判 60 个正类样本，漏判率为 22.81%。实验组识别出 234 个正类样本，漏判数降至 29 个，漏判率为 11.03%。也就是说，实验组相比对照组少漏判 31 个高风险样本，漏判数量下降 51.67%。这是一项比较明显的改进。

实验组的代价是 FP 从 42 个增加到 54 个，误报率从 7.18% 上升到 9.23%。不过，实验组 Precision 仍为 0.8125，说明被判定为高风险的样本中仍有较高比例是真实正类。结合 Recall、F1 和 Accuracy 的提升可以看出，`f1` 策略在本文数据集上取得了更合适的综合效果。

因此，本文最终采用实验组，即 `threshold_search_mode=f1` 的 LightGBM 风险分类方案。该选择并不是因为实验组的 AUC 更高，而是因为两组 AUC 相同、模型排序能力相同的情况下，`f1` 策略能够明显减少高风险样本漏判。对于健康管理提醒系统而言，漏掉潜在高风险用户比多提醒少量低风险用户更不利，因此 `f1` 策略更符合本文系统的使用场景。`recall_priority` 组可作为更保守的备选方案，适用于后续更强调降低误报的场景。

#### 5.2.4 本节小结

本节通过重新运行 `recall_priority` 和 `f1` 两组实验，对 LightGBM 风险分类结果进行了比较。实验表明，两组在数据集、调参结果、AUC、PR-AUC 和 Brier Score 上保持一致，差异主要来自阈值搜索策略。`f1` 实验组在 Precision 小幅下降的情况下，将 Recall 从 0.7719 提升到 0.8897，将 F1 从 0.7992 提升到 0.8494，并将 FN 从 60 个降到 29 个。综合考虑本文系统的健康风险提醒定位，最终选择 `threshold_search_mode=f1` 作为 LightGBM 模型的阈值策略。
...
为验证两个模型能够在实际预测流程中完成衔接，本文对“输入数据 -> Prophet 趋势预测 -> LightGBM 风险分类 -> 趋势融合 -> 页面展示”的关键节点进行流程检查。验证重点不是覆盖系统全部功能，而是确认预测流程在系统页面中能够被正确调用和展示。结果如表 5-8 所示。
...
验证结果表明，Prophet 与 LightGBM 的串联关系能够在实际预测流程中完整运行。Prophet 输出的未来 7 天血压预测点能够转化为 LightGBM 输入特征，LightGBM 原始概率能够进一步结合趋势信号形成最终展示结果，预测页面也能呈现风险概率、趋势图和健康建议。因此，系统部分在本文中的作用是验证双模型预测流程可以落地运行，而不是作为独立的管理系统展开讨论。

### 5.7 本章小结

本章介绍了实验环境、LightGBM 训练结果、特征重要性、Prophet 预测精度回测、趋势融合效果分析和双模型预测流程验证。实验结果表明，LightGBM 在测试集上具有较好的分类效果，血压相关特征是模型最主要的判别信号；Prophet 在历史数据达到 30 天以上时，7 天预测收缩压 MAE 在 3-7 mmHg 范围内；趋势融合对高血压场景和服药未控制场景产生向上修正，调整幅度在 [0, +0.12] 区间内。流程验证说明，Prophet 预测期血压特征能够顺利进入 LightGBM 风险分类模型，并在页面中形成可理解的预测结果。
...
在预测流程实现方面，本文围绕两个模型完成了风险因素输入、血压记录输入、7 天风险预测、风险结果展示和指南型健康建议生成。系统页面主要用于验证预测流程落地：用户发起预测后，后端完成 Prophet 趋势预测、LightGBM 风险分类和趋势融合，前端展示风险概率、风险等级和未来 7 天血压趋势。
