...
高血压风险预测研究近年来逐渐从传统统计模型扩展到机器学习模型。国内已有研究对高血压患病风险预测模型进行了综述，指出风险预测通常需要综合人口学特征、生活方式因素、血压指标和代谢指标等多类变量[5]。在具体应用方面，钢铁工人高血压风险预测模型研究比较了多种机器学习模型，并使用 AUC、F1、Brier Score 等指标评价模型表现[6]；基于易采集风险因素的高血压预测研究以及原发性高血压不良心血管事件风险预测研究，则体现出风险预测任务在基础筛查和多模态扩展场景中的应用价值[7-8]。
...
输入：用户历史血压记录 `BP_history`、个人风险因素 `X_risk`。

输出：融合风险概率 `p_fused`、风险等级和健康建议。

1. 读取用户历史血压记录和个人风险因素，完成基础字段校验。
2. 按自然日聚合收缩压和舒张压记录，形成 Prophet 可处理的日均血压序列。
3. 分别使用 Prophet 预测未来 7 天收缩压和舒张压，得到预测期血压点。
4. 计算预测期收缩压均值和舒张压均值，记为 `SBP_future` 和 `DBP_future`。
5. 将预测期血压特征与个人风险因素合并，形成 LightGBM 输入向量。
6. 调用 LightGBM 风险分类模型，得到原始风险概率 `p_raw`。
7. 从预测期血压点中提取高血压天数比例、血压峰值和趋势方向等融合信号。
8. 根据趋势融合规则对 `p_raw` 进行小幅修正，得到融合风险概率 `p_fused`。
9. 将 `p_fused` 映射为风险等级，并结合趋势和置信度生成健康建议。
...
为验证不同阈值搜索策略对系统实际应用效果的影响，本文基于调参后的模型结构，设计了一组对照实验：对照组采用 `recall_priority` 策略（设置 Recall 下限约束为 0.75），实验组采用 `f1` 策略（直接搜索最大化 F1 值的阈值，无 Recall 下限限制）。两组在相同数据划分和相同调参结果下进行训练，差异主要来自最终分类阈值选择，实验结果对比见表 5-4。
...
从表 5-4 可以看出，模型本身的排序区分能力（AUC=0.9480）不受阈值策略影响。当采用 `f1` 策略时，模型倾向于选择较低的阈值（0.6676），使 Recall 从 0.7719 提升到 0.8897，F1 从 0.7992 提升到 0.8494；Precision 从 0.8286 小幅下降到 0.8125，但仍保持在 0.80 以上。对应混淆矩阵中，`f1` 策略将 FN 从 60 降至 29，说明其能够减少潜在高风险样本被漏掉的情况。
