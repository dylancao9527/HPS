...
```latex
g(t)=(k+a(t)^{T}\delta)t+(m+a(t)^{T}\gamma)#(2)
```
...
```latex
\mathcal{L}^{(m)}=\sum_{i=1}^{N}l(y_i,\hat{y}^{(m-1)}_i+f_m(x_i))+\Omega(f_m)#(4)
```
...
```latex
p=P(y=1\mid x)=\frac{1}{1+e^{-F_M(x)}}#(6)
```
...
```latex
\overline{SBP}_d=\frac{1}{n_d}\sum_{i=1}^{n_d}SBP_{d,i}#(7)
```
...
```latex
\overline{DBP}_d=\frac{1}{n_d}\sum_{i=1}^{n_d}DBP_{d,i}#(8)
```
...
```latex
SBP_{future}=\frac{1}{7}\sum_{k=1}^{7}\widehat{SBP}_{t+k}#(9)
```
...
```latex
DBP_{future}=\frac{1}{7}\sum_{k=1}^{7}\widehat{DBP}_{t+k}#(10)
```
...
```latex
F1=\frac{2\times Precision\times Recall}{Precision+Recall}#(14)
```
...
```latex
Brier=\frac{1}{N}\sum_{i=1}^{N}(p_i-y_i)^2#(15)
```
...
```latex
X_{bp}^{future}=\left[\frac{1}{7}\sum_{k=1}^{7}\widehat{SBP}_{t+k},\frac{1}{7}\sum_{k=1}^{7}\widehat{DBP}_{t+k}\right]#(16)
```
...
```latex
\begin{aligned} \mathbf{x}_{LGBM}=&(male,age,smoker,cigsPerDay,BPMeds,diabetes,\\ &totChol,SBP_{future},DBP_{future},BMI,heartRate,glucose),\\ p_{raw}=&F_{LGBM}(\mathbf{x}_{LGBM}) \end{aligned}#(17)
```
...
```latex
r_{high}=\frac{1}{7}\sum_{k=1}^{7}\mathbb{I}(\widehat{SBP}_{t+k}\ge140\ \text{or}\ \widehat{DBP}_{t+k}\ge90)#(18)
```
...
```latex
s_{SBP}=\widehat{SBP}_{t+7}-\widehat{SBP}_{t+1},\quad s_{DBP}=\widehat{DBP}_{t+7}-\widehat{DBP}_{t+1}#(19)
```
...
```latex
\Delta_{trend}=\min(0.08,0.08r_{high})+0.02I_{peak}+0.02I_{up}-0.02I_{stable}#(20)
```
...
```latex
p_{fused}=clip(p_{raw}+\Delta_{trend}+\Delta_{med},0.01,0.99)#(21)
```
...
```latex
Level(p)= \begin{cases} 低风险,&p<0.30\\ 中风险,&0.30\le p<0.60\\ 高风险,&p\ge0.60 \end{cases}#(22)
```
