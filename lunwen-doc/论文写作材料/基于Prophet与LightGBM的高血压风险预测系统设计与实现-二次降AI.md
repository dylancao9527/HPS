...
## DESIGN AND IMPLEMENTATION OF A HYPERTENSION RISK PREDICTION SYSTEM BASED ON PROPHET AND LIGHTGBM
## Abstract

Hypertension remains one of the most prevalent and critical challenges in contemporary chronic disease management globally. With the rapid proliferation of smart health devices, home blood pressure monitors and digital health platforms continuously accumulate massive volumes of personal measurement records. However, a significant gap exists between data collection and actionable insight. In current practice, these extensive records predominantly serve only rudimentary storage and basic querying needs. Advanced analytics—such as recent blood pressure trend analysis, dynamic short-term risk reminders, and transparent explanations for prediction results—have not yet been successfully integrated into daily personal health routines. Consequently, ordinary users are often confronted with a dense, confusing series of numerical values. Without professional medical training, they find it exceptionally difficult to accurately evaluate whether their daily systolic and diastolic blood pressure fluctuations are benign or if they represent a severe health anomaly demanding immediate attention.To bridge this critical gap, this paper details the design and implementation of a novel hypertension risk prediction assistance system, leveraging a hybrid machine learning architecture based on Prophet and LightGBM. The data pipeline begins with meticulous preprocessing: raw, sporadically collected user blood pressure data are systematically organized into daily average sequences on a natural day basis to smooth out noise. This structured time-series data is then fed into Prophet, a robust forecasting tool capable of handling missing values and daily seasonality, which generates precise seven-day future trends for both systolic and diastolic blood pressure.The system extends beyond simple time-series forecasting. Features derived from the Prophet model, such as the predicted short-term blood pressure averages and trend slopes, are strategically combined with established physiological and lifestyle risk factors. These essential covariates include the user's age, Body Mass Index (BMI), smoking status, current medication use, and the presence of comorbid conditions like diabetes. This comprehensive feature set is subsequently processed by LightGBM, a highly efficient gradient boosting framework. LightGBM synthesizes these dynamic forecasts and static variables to output a specific hypertension risk probability alongside a clearly categorized risk level (e.g., low, moderate, or high risk).Extensive experimental results validate the efficacy of this hybrid approach. When evaluated against a comprehensive public health-check dataset, the carefully parameter-tuned LightGBM model achieves exceptional classification and predictive performance. Feature importance analysis confirms that the engineered blood pressure features appropriately carry the most significant weight during the model's decision-making process. Furthermore, Prophet back-testing and trend-fusion analyses conclusively demonstrate that short-term blood pressure trends can be seamlessly transformed into discrete risk classification inputs. This integration forms bounded, highly traceable adjustments upon the original baseline risk probability, ensuring algorithmic transparency for the user. Ultimately, while this proposed methodology proves highly suitable as a proactive reference tool for short-term personal health management and early intervention, it is explicitly designed to empower users and complement—rather than substitute—formal clinical diagnosis, professional medical treatment decisions, or routine physician consultations.
