...
import { getLatestMockEmail } from '@/config/api'
...
function ProfileFormContent({ profileUser, sections }: ProfileFormContentProps) {
  ...
  const [accountForm, setAccountForm] = useState<AccountFormState>(() => getAccountFormState(profileUser))

  const [healthForm, setHealthForm] = useState<HealthFormState>(() => getHealthFormState(profileUser))
  ...
  const handleSendChangeEmailCode = async () => {
    ...
    try {
      ...
      const mockEmail = data.mock_service === 'local_email'
    ...
    <div>
      {visibleSections.has('account') && (
        <AccountProfileSection
          ...
          onSendChangeEmailCode={handleSendChangeEmailCode}
      ...
      {visibleSections.has('riskFactors') && (
        <HealthProfileSection
          ...
          bmi={bmi}
          bmiStatus={bmiStatus}
          bmiToneClass={bmiToneClass}
          smokerEnabled={smokerEnabled}
          savingHealth={savingHealth}
          onUpdateField={updateHealthField}
      ...
      {visibleSections.has('password') && (
        <PasswordSettingsSection
          ...
          onChangeMode={setPasswordMode}
          onUpdatePasswordField={updatePasswordField}
          onUpdateResetPasswordField={updateResetPasswordField}
          onChangePassword={handleChangePassword}
          onSendResetCode={handleSendResetCode}
          onResetPasswordByEmail={handleResetPasswordByEmail}
        />
      )}
    </div>
  )
