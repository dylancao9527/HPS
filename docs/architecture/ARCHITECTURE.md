...
uv run python train_models.py --seed 7
uv run python train_models.py --random-seed
uv run python train_models.py --save-params configs/my_params.json
uv run python train_models.py --params configs/my_params.json
```
...
```bash
cd backend
uv run python export_training_data.py
```
...
5. 保存产物：写入 `lgbm_model.txt`、`model_config.json`、`training_meta.json` 和 `docs/model_report.md`

终端输出默认只展示阶段状态、样本量、CV AUC、最终 AUC / Recall / F1、阈值、保存路径和本次实际使用的 seed。详细参数、完整指标和特征重要性进入 `docs/model_report.md` 与 `training_meta.json`，避免训练过程刷屏。
...
```bash
# 1. 导出默认参数作为基线快照
uv run python train_models.py --save-params configs/baseline.json

# 2. 修改 configs/experiment.json 中的参数（如 max_boost_rounds: 3000）

# 3. 用不同参数分别训练
uv run python train_models.py --params configs/baseline.json --seed 42
uv run python train_models.py --params configs/experiment.json --seed 42

# 4. 比较 backend/ml_models/training_meta.json 或 docs/model_report.md
```
