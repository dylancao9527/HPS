...
- 提交区间：`652df0572920c53945dfb670b4d1277ee63391a1..1fb193487b69562f1bfd0f5e8537b36368191560`，包含 Prophet 持久化精简、预测/治理分页边界、训练脚本与前端结构修复、路由/服务/脚本安全加固。
- Prophet 预测周期已固定为 7 天，命令层与训练上下文不再保留可变 `forecast_period_days` 口径；论文中应统一为“未来 7 天预测”，避免写成可配置预测天数。
- `user_prophet_models` 表已精简，当前模型更偏“用户级 Prophet 模型资产索引”：保留用户、模型版本、数据签名、训练时间、训练截止日期、存储键和活跃状态，不再保留过多运行期说明属性。
- 预测记录采用紧凑 JSON 载荷与普通列混合保存，并有 `MAX_PAGE_SIZE=100` 的分页/limit 边界；论文数据库设计与测试说明可补充“列表查询限幅”。
- 认证与本地模拟邮箱接口增加后端保护：本地模拟邮箱默认关闭、接口要求本机或开发令牌、验证码生成使用安全随机数并限制错误次数；论文系统安全设计可补充该工程约束。
- demo 用户脚本默认预览，不写库；实际写入需 `--apply`，删除需二次确认；论文测试/部署说明可写为“演示数据脚本具备防误操作参数”。
- 本轮 ER 图只更新 `.drawio` 源文件，不转出图片。

## 2026-05-11 论文待改点

- 主论文 §2.2 与 §4.1 仍把“训练数据天数、置信度等级、参数画像、模型运行模式”整体描述为 Prophet 模型元数据保存，需区分：模型资产表只保存版本、签名、训练时间、训练截止日期、存储键和活跃状态；训练说明类信息保存在预测记录 JSON 载荷中。
- 主论文 §5.4 仍写 `history_window_capped` 进入训练元信息。当前更准确的表述是：窗口截断、参数画像和置信度说明进入预测记录的 `training_meta`，不作为 `user_prophet_models` 字段。
- 主论文 §5.1 可补充训练脚本现在把 `max_boost_rounds`、`early_stopping_rounds` 透传给 `LightGBMTunerCV`，并且数据集 hash 在排序后生成以避免 seed 扰动。
- 主论文 §5.6 系统测试表可增加输入边界与安全边界测试：非法 `recorded_at` 返回 400、分页/limit 限幅、验证码失败锁定、本地模拟邮箱后端访问闸门。
- 图片清单当前图4-5 文件列仍指向 `.png`，但用户本轮要求 ER 图只生成 drawio；需要将本轮说明改为“本轮只更新源文件，不重新导出图片”。
